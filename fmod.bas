...
'Added for Simple Amp
Public Declare Function FSOUND_Stream_GetTagField2 Lib "fmod.dll" Alias "_FSOUND_Stream_GetTagField@24" (ByVal stream As Long, ByVal num As Long, ByRef tagtype As Long, ByRef name As Long, ByRef value As Long, ByRef Length As Long) As Byte
...
Public Function GetStringFromPointerEx(ByVal lpString As Long, ByVal lLength As Long) As String
  'Added for Simple Amp by Paul Berlin
  'Use this to get a string from pointer when there may be chr(0)
  'not only at the end of the string.
  Dim abBuffer() As Byte
  
  ReDim abBuffer(lLength)
  CopyMemory abBuffer(0), ByVal lpString, lLength

  GetStringFromPointerEx = StrConv(abBuffer, vbUnicode)
...
Public Function GetSpectrum(ByRef Spectrum() As Single)
  Dim nrOfVals As Long, lpSpectrum As Long, A As Long
...
'These functions added by Paranoid_Android.
Public Function GetScopeINT(ByRef Scope() As Integer)
    
    Dim nrOfVals As Long
    If UBound(Scope) > 1023 Then nrOfVals = 1024 Else nrOfVals = UBound(Scope) + 1
    CopyMemory Scope(0), ByVal RealtimeBuffer, nrOfVals * 2
...
Public Function GetScopeFPU(ByRef Scope() As Single)
    ...
    Dim nrOfVals As Long
    If UBound(Scope) > 1023 Then nrOfVals = 1024 Else nrOfVals = UBound(Scope) + 1
    CopyMemory Scope(0), ByVal RealtimeBuffer, nrOfVals * 4
